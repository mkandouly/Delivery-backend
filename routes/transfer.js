...
transferRoute.post("/", isLoggedIn, isCancel, async (req, res) => {
  ...
  const newtrans = await pool.query(
    ...
    [decoded.empid, driverid, "RETURN_DEPOT"],
  );
  ...
  SELECT $1, UNNEST($2::int[]), 'RETURN_DEPOT'
  `,
