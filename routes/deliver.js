...
import { isLoggedIn, isDeliver } from "../middleware/middleware.js";

const deliverRoute = Router();

deliverRoute.post("/", isLoggedIn, isDeliver, async (req, res) => {
  ...
  const newtrans = await pool.query(
    ...
    [decoded.empid, driverid, "DELIVERED"],
  );
  ...
  const result = await pool.query(
    `
      INSERT INTO delivered (orderid, tid)
